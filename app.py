...
def detalles_paciente():
    ...
    try:
        ...
        cursor.execute('SELECT * FROM historia_salud_paciente WHERE id_paciente = ?', (id_paciente,))
        historia_salud = cursor.fetchone()

        cursor.execute('SELECT * FROM tipificacion_adulto_mayor WHERE id_paciente = ?', (id_paciente,))
        tipificacion = cursor.fetchone()
    ...
    if area == 'coordinacion':
        return render_template(f'{area}/detalles_paciente.html', paciente=paciente, antecedentes_personales_nutricion=antecedentes_personales_nutricion,
                               ...
                               evaluacion_bioquimica=evaluacion_bioquimica, historia_salud=historia_salud, tipificacion=tipificacion)
    else:
        return render_template(f'{area}/detalles_paciente.html', paciente=paciente, antecedentes_personales_nutricion=antecedentes_personales_nutricion,
...
@app.route('/registro_historia_salud', methods=['GET', 'POST'])
@login_required
def registro_historia_salud():
    area = session.get('area')

    if request.method == 'POST':
        id_paciente = request.form['id_paciente']

        # Consolidar campos del formulario
        historia_salud = {
            'enfermedades': request.form['enfermedades'],
            'antecedentes_heredofamiliares': request.form['antecedentes_heredofamiliares'],
            'terapeutica': request.form['terapeutica'],
            'traumas': request.form['traumas'],
            'cirugias': request.form['cirugias'],
            'alergias': request.form['alergias'],
            'toxicomanias': request.form['toxicomanias'],
            'sueno': request.form['sueno'],
            'estado_nutricional': request.form['estado_nutricional'],
            'perdidas': request.form['perdidas'],
            'perdidas_funcionales': request.form['perdidas_funcionales'],
            'ansiedad': request.form['ansiedad'],
            'depresion': request.form['depresion'],
            'personalidad': request.form['personalidad'],
            'otro_psicologico': request.form['otro_psicologico'],
            'redes_apoyo': request.form['redes_apoyo'],
            'nivel_socioeconomico': request.form['nivel_socioeconomico'],
            'actividad_economica': request.form['actividad_economica'],
            'actividad_ocio': request.form['actividad_ocio'],
            'rol_social': request.form['rol_social'],  # Nuevo campo para el rol social
            'red_familiar': request.form['red_familiar'],  # Nuevo campo para la red familiar
        }

        try:
            with get_db_connection() as connection:
                cursor = connection.cursor()
                # Verificar si el id_paciente ya tiene registros en historia_salud_paciente
                cursor.execute("SELECT id_paciente FROM historia_salud_paciente WHERE id_paciente = ?", (id_paciente,))
                if cursor.fetchone():
                    # Si existe, actualizar los datos
                    update_fields = ", ".join([f"{field}=?" for field in historia_salud.keys()])
                    query = f"UPDATE historia_salud_paciente SET {update_fields} WHERE id_paciente=?"
                    cursor.execute(query, (*historia_salud.values(), id_paciente))
                else:
                    # Si no existe, insertar los datos
                    fields_str = ", ".join(historia_salud.keys())
                    values_str = ", ".join(['?' for _ in historia_salud])
                    query = f"INSERT INTO historia_salud_paciente (id_paciente, {fields_str}) VALUES (?, {values_str})"
                    cursor.execute(query, (id_paciente, *historia_salud.values()))
                connection.commit()
            flash("Historia de salud registrada exitosamente", "success")
        except sqlite3.Error as e:
            flash(f"Error en la base de datos: {e}", "danger")
            return redirect(url_for('registro_historia_salud'))
    ...
    return render_template('fisioterapia/registro_historia_salud.html', pacientes=pacientes, area=area)

@app.route('/tipificacion_adulto_mayor', methods=['GET', 'POST'])
@login_required
def tipificacion_adulto_mayor():
    if request.method == 'POST':
        id_paciente = request.form['id_paciente']
        vejez = request.form['vejez']
        envejecimiento = request.form['envejecimiento']
        adulto_mayor = request.form['adulto_mayor']
        indice_comorbilidad_mortalidad = request.form['indice_comorbilidad_mortalidad']
        capacidad_fisica_funcional_total = request.form['capacidad_fisica_funcional_total']
        movilidad_miembros_superiores = request.form['movilidad_miembros_superiores']
        movilidad_miembros_inferiores = request.form['movilidad_miembros_inferiores']
        movilidad_tronco = request.form['movilidad_tronco']

        with get_db_connection() as connection:
            cursor = connection.cursor()
            cursor.execute('''INSERT INTO tipificacion_adulto_mayor (
                id_paciente, vejez, envejecimiento, adulto_mayor, indice_comorbilidad_mortalidad,
                capacidad_fisica_funcional_total, movilidad_miembros_superiores, movilidad_miembros_inferiores, movilidad_tronco
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''', (
                id_paciente, vejez, envejecimiento, adulto_mayor, indice_comorbilidad_mortalidad,
                capacidad_fisica_funcional_total, movilidad_miembros_superiores, movilidad_miembros_inferiores, movilidad_tronco
                ))
    ...
    return render_template('fisioterapia/tipificacion_adulto_mayor.html', pacientes=pacientes)    
